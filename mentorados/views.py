...
from .models import DisponibilidadeHorarios, Navigators, Mentorados, Reuniao, Tarefa, Upload
...
def tarefa_mentorado(request):
    ...
    if request.method == 'GET':
        videos = Upload.objects.filter(mentorado=mentorado)
        tarefas = Tarefa.objects.filter(mentorado=mentorado)
        return render(request, 'tarefa_mentorado.html', {'mentorado': mentorado, 'tarefas': tarefas, 'videos': videos})
